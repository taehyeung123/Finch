-- 0073_slug_reserved_profile_link.sql — 예약어에 'profile-link' 추가 (2026-08-29)
--
-- 프로필 링크 마케팅 페이지(app/(finch)/(marketing)/profile-link)를 새로 만들었다.
-- Next.js 는 정적 경로가 동적 경로를 이기므로, 누군가 'profile-link' 를 주소로 쓰고 있으면
...
-- 충돌 행은 임시 무작위 주소로 옮기고 slug_set_at 을 비워 «주소 정하기» 를 다시 받는다
-- (주소를 뺏었으니 최초 1회 권한과 쿨다운을 돌려준다 — 0068 과 같은 처리).
update public.link_pages
set slug = substr(md5(random()::text || id::text), 1, 8), slug_set_at = null
where slug = 'profile-link';
...
alter table public.link_pages add constraint link_pages_slug_not_reserved check (
  slug not in (
    ...
    'brand','features','goodbye','instagram','pricing','privacy','profile-link','reference','terms','threads','tiktok',
    -- 앱(로그인 후)
