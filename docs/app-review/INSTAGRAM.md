...
- Every Korean label in «…» is the exact string from the code (checked against the repo on 2026-09-11). The text in parentheses is an English gloss. The **Label index** at the end gives file:line for each label.
...
12. Use a small square JPEG, under 1 MB. The composer resizes each photo to at most 1440 px as JPEG. It refuses a batch over about 3 MB after resizing with «사진 용량 합계가 커요 — 몇 장을 빼고 다시 시도해 주세요.» (The photos are too large in total), and it crops anything outside 4:5 to 1.91:1.
...
- **D7 (minor):** The composer's channel row shows «틱톡» with a «준비 중» (coming soon) tag (`post-composer.tsx:523`). This is low risk; noted for completeness.
...
### How we use it
Finch uses instagram_business_content_publish so a user can post photos to their own connected Instagram account from «발행» (Publish). In the «새 게시물 포스팅» (New post) composer the user picks «인스타그램», adds 1–10 images, writes the «캡션» (Caption) and chooses «지금 발행» (Publish now), «예약 발행» (Schedule) or «초안으로 저장» (Save as draft). Finch converts the images to JPEG and crops them into Instagram's 4:5–1.91:1 range. Nothing is published until the user clicks «지금 발행하기» (Publish now) or the time they chose arrives. To publish, Finch uploads the images to its own storage so Instagram can fetch them by URL, creates a single-image container (or one item container per image plus a CAROUSEL container) with `POST /{ig-user-id}/media`, polls the container's `status_code` until it is `FINISHED`, and then calls `POST /{ig-user-id}/media_publish`. The result appears in a dialog and the post moves to the «발행완료» (Published) tab. Scheduled posts go through the same calls within 5 minutes of the chosen time, and Finch never edits or deletes posts on Instagram.
...
5. «채널» (Channel): «인스타그램» is already selected. Under «이미지» (Images), click «추가» (Add) and choose a JPG or PNG. Square works best.
6. Type a caption, hashtags included, in «캡션» (Caption).
7. Under «발행 방식» (Publishing method), the default is «예약 발행» (Schedule). Select «지금 발행» (Publish now), then click **«지금 발행하기»** (Publish now). It can take up to about a minute.
8. The dialog «인스타그램에 올라갔어요» (Posted to Instagram) appears. Click «확인» (OK). Then click the tab «발행완료» (Published); the post shows «발행 완료».
9. Open https://www.instagram.com/sding.kr/. The new post is first. You can delete it on Instagram afterwards; Finch doesn't delete posts.
...
| 6 | Click «새 게시물 포스팅» (New post). Dialog «새 게시물 포스팅», «채널» «인스타그램» selected | Create a new post for Instagram. |
| 7 | «이미지» → «추가» (Add) → square JPEG. The counter reads «1/10» | Add a photo. Finch converts it to JPEG within Instagram's ratio limits. |
| 8 | «캡션»: type `Posted from Finch for Meta app review #finch #test` | Write the caption and hashtags. |
| 9 | «발행 방식»: switch from the default «예약 발행» to «지금 발행». Its description reads «저장하자마자 바로 올라가요. 올라간 뒤엔 여기서 되돌릴 수 없어요.» | Choose "지금 발행" (Publish now). Nothing is posted until the user clicks. |
| 10 | Click «지금 발행하기». The button shows «발행 중…» with «인스타그램이 게시물을 처리하는 동안 잠시 걸릴 수 있어요. 창을 닫지 마세요.» | Finch creates a media container, waits until Instagram finishes processing it, then calls media_publish. |
| 11 | Dialog «인스타그램에 올라갔어요» / «「발행완료」 탭에서 확인할 수 있어요.» → «확인» | Instagram confirmed the post. |
| 12 | Click the tab «발행완료» (Published): a row with thumbnail, caption, time and «발행 완료» | The post is listed under "발행완료" (Published). |
| 13 | Profile 3: instagram.com/sding.kr → refresh → the new post is first → open it and show the caption and hashtags | On Instagram, the same photo and caption are live on @sding.kr's feed. |
...
- Publish: `publish/page.tsx:121` «발행» · `publish/_components/publish-list.tsx`: strip «인스타그램» 307 · «캘린더» 352 · «발행완료» 354 · «새 게시물 포스팅» 383 · `publish/_components/post-composer.tsx`: «새 게시물 포스팅» 485 · «채널» 499 · «준비 중» 523 · «이미지» 533 · «{n}/10» 537 · «추가» 578 · crop note 594 · caption label 612 (= «캡션», `lib/publish-rules.ts:55`) · «발행 방식» 631 · «지금 발행» 646-647 · «예약 발행» 663 · «초안으로 저장» 691 · «발행 중…» 708 · «지금 발행하기» 713 · processing note 719 · «{label}에 올라갔어요» 405 (label «인스타그램», `lib/publish-rules.ts:20`) · `components/ui/status-pill.tsx:25` «발행 완료»
...
- API calls: `lib/meta/instagram-oauth.ts:207-213, 241-248` · `lib/meta/instagram.ts:77-122, 147-170, 187-211, 294-297, 317-321, 358-364, 404-415` · `lib/meta/instagram-publish.ts:71-125` · `lib/meta/graph.ts:18, 106-160` · `lib/auto-dm/pipeline.ts`, `lib/auto-dm/check-now.ts`, `lib/auto-dm/match.ts:26-60` · `app/api/webhooks/instagram/route.ts:153-175` · `supabase/migrations/0092_auto_dm_manual_checks.sql:110-212`
