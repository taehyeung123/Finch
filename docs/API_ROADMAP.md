...
**⚠️ 마이그레이션은 87개다**(`0001`~`0087`, 2026-09-08 기준 전부 적용됨). 옛 문서가 «0001~0003이면 된다»고 적어 두었는데
그것만 적용하면 결제·예약발행·틱톡·팀·크레딧·레퍼런스·공용 풀·프로필 링크가 전부 없는 반쪽 DB가 된다.

`0085`~`0087` 은 2026-09-07 보안 감사 수리다 — 토큰 암호문 권한 회수(0085), 페이지 재부모화·감사 로그 이전 차단(0086),
방침이 약속한 파기 경로 신설(0087). **0085 는 코드 배포 뒤에 적용해야 한다**(그 컬럼을 읽던 코드를 같은 배포에서
service_role 경로로 옮겼다).
