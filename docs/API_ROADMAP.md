...
**⚠️ 마이그레이션은 95개다**(`0001`~`0095`, 2026-09-12 기준 — 0091·0092·0093·0094·0095 는 SQL Editor 적용 대기, 0092 는 자동 DM «지금 확인» 남용 제한, 0093 은 발행 사진·영상·처리 중 상태, 0094 는 발행 글의 대상 계정(계정을 바꾸면 옛 예약을 새 계정으로 올리지 않기), 0095 는 동의·철회 이력 + 메타 삭제 요청의 후속 삭제 대상 + 보존 파기 인덱스(약관·방침 정식본, docs/LEGAL_REVIEW_2026-09.md) — 0093·0094 는 코드보다 먼저 적용, 0095 는 배포 전 적용 권장·늦어도 잠김 없음). 옛 문서가 «0001~0003이면 된다»고 적어 두었는데
그것만 적용하면 결제·예약발행·틱톡·팀·크레딧·레퍼런스·공용 풀·프로필 링크가 전부 없는 반쪽 DB가 된다.

`0085`~`0090` 은 2026-09-07~08 보안 감사 수리다 — 토큰 암호문 권한 회수(0085), 페이지 재부모화·감사 로그 이전 차단(0086),
방침이 약속한 파기 경로 신설(0087), 광고 계정 id 잠금(0088), 고아 파일 «관찰» 표(0089),
댓글 작성자 해시 페퍼 전환(0090 — 데이터는 안 바꾸고 조회가 옛 해시도 함께 보게 한다).
**0085 와 0088 은 코드 배포 뒤에 적용해야 한다** — 그 컬럼을 만지던 코드를 같은 배포에서 service_role 경로로 옮겼다.
...
## 사람이 해야 할 일 (2026-09-03 기준)

**끝난 것** — Supabase Pro·프로젝트 생성 · Anthropic 키 · ScrapeCreators 키 ·
개인정보처리방침 게시 · Vercel 배포 · 도메인(finch.ai.kr) · 네이버·구글 사이트등록 ·
구글 OAuth 브랜딩·인증 · **사업자등록**(주식회사 딥레드, 2026-08-31 개업 — 값은 `lib/legal/business.ts`) ·
Supabase 수동 연결(manual linking) ON(2026-09-03)
...
- [ ] **Vercel 플랜 확인** — 지금 코드는 Hobby 제약에 맞춰져 있다(크론 하나가 하루 1회만
      ...
      → **2026-09-09 적용됨.** 발행 크론은 5분마다 돌고(`vercel.json`), 예약은 «시각»을 받으며 「지금 발행」이 생겼다(`lib/publish/run.ts`).
