...
- 비우기는 `revalidatePath`(즉시 만료)다. `revalidateTag(…, "max")` 는 옛 화면을 한 번 더 주므로 쓰지 않는다.
  운영 실측(2026-09-11, /profile 을 운영자 비우기로 비움): 바로 다음 요청은 새로 구운 화면(`REVALIDATED`)이었고,
  그 뒤 몇 초 동안 일부 CDN 서버가 옛 화면을 한 번 더 줬다(`STALE`, 뒤에서 갱신). 20초 안에 모든 요청이 새 화면(`HIT`)이었다.
  그래서 «즉시»는 «몇 초 안에»로 말한다.
