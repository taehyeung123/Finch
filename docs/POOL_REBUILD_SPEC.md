...
> ## ⚠️ 이 스펙은 **일부만 구현됐다** — 「구현 스펙」이라는 제목을 그대로 믿지 말 것 (2026-08-31 대조)
>
> **구현된 것:** 공용 풀 전환 자체(`lib/pool/**`, 마이그레이션 0027~0035, pool-plan/pool-work/pool-finalize 크론),
> 사용자별 자동수집 폐기, 업종 분류(`lib/industry/**`).
>
> **구현되지 않은 것 — 본문이 «신설한다»고 적었지만 코드에 없다:**
>
> | 스펙이 말하는 것 | 실제 |
> |---|---|
> | 라우트 `/industry` · `/industry/[key]` · `/brand/[id]` | **없다** (`app/` 아래 해당 라우트 0건) |
> | 사이드바에 «업종별 트렌드» 추가, `/library` 라벨을 «레퍼런스 검색»으로 | 라벨은 여전히 **«탐색»** |
> | 크론 4종 (pool-plan 03:05 / pool-work `*/10` 18-20 / **pool-enrich** 06:10 / pool-finalize 06:40) | pool-enrich 는 **라우트조차 없다**. 실제 스케줄은 pool-plan `0 20` / pool-work 8회(3시간 간격) / pool-finalize `30 18` |
> | 업종 24개 (beauty/sup… 키) | **22개**이고 키가 다르다 (`lib/industry/list.ts`) |
> | 크론 라우트 전부 `maxDuration=300` | 실제는 **60**(Hobby 상한) |
>
> 본문은 **당시의 설계 의도**로 읽고, 지금 무엇이 도는지는 `vercel.json` · `lib/pool/**` ·
> `docs/POOL_OPERATIONS.md`(운영 정본)를 볼 것.
