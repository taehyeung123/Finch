...
/**
 * 페이지 비밀번호(잠금 페이지)를 **새로 정할 때**의 최소 길이.
 * 화면·서버 액션·검증 함수가 같은 값을 봐야 해서 여기 둔다(lib/links/password.ts 는 node:crypto 를
 * 쓰므로 편집기 화면에서 못 읽는다).
 * 4에서 6으로 올린 이유: 4자리 숫자는 경우의 수가 1만이라 10분 8회 상한으로도 이틀이면 뚫린다(2026-09-07 감사).
 * ⚠️ 이미 저장된 4~5자 비밀번호는 그대로 동작한다 — 해제 경로는 이 값을 보지 않는다.
 */
export const MIN_PAGE_PASSWORD = 6;
