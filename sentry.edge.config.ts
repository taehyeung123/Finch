...
Sentry.init({
  ...
  /* ⚠️ 아웃고잉 fetch 브레드크럼을 끈다 — 서버 설정과 같은 이유다.
     Graph 호출은 토큰을 쿼리스트링에 담고(`?access_token=…`) OAuth 교환은 client_secret 을 보내는데,
     브레드크럼은 요청 URL 을 통째로 기록한다. 오류 하나에 딸려 그 URL 이 외부 로그로 나간다.
     edge 의 동등물은 winterCGFetchIntegration 이다(@sentry/vercel-edge → @sentry/nextjs 재export).
     지금 edge 로 도는 코드는 없지만, 하나 생겼을 때 조용히 빠지지 않게 미리 건다(2026-09-07 감사). */
  integrations: [Sentry.winterCGFetchIntegration({ breadcrumbs: false })],
