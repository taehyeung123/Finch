...
export function SubscribeButton({ label, inline = false }: { label: string; inline?: boolean }) {
  return (
    <button
      ...
      className={`${inline ? "relative" : "absolute left-5 top-4 z-20"} inline-flex h-9 items-center gap-1.5 rounded-full bg-[var(--lp-accent)] px-3.5 text-[13px] font-semibold text-[var(--lp-on-accent)] shadow-[var(--lp-shadow)] transition-opacity hover:opacity-85`}
    >
