...
/**
 * TikTok 연동 시작 — 로그인 사용자 확인 후 TikTok 인가 화면으로 리다이렉트.
 * app/api/auth/threads/start/route.ts와 동일 구조(CSRF state 쿠키).
 * PKCE 미사용 결정 근거: lib/tiktok/oauth.ts 상단 주석 참고.
...
export async function GET(request: Request) {
  ...
  if (isChannelClosed("tiktok", user.email)) {
    return NextResponse.redirect(`${origin}/settings/channels?connect=unconfigured`);
  }
  ...
  const authorizeUrl = buildTiktokAuthorizeUrl({
    ...
    redirectUri: resolveTiktokCallbackUri(request),
    state,
  });
