  ...
  return (
    <div
      className={cn(
        ...
        /* 바닥 280px — 1536×864(1080p·125% 배율, 가장 흔한 노트북)에서 폰·주소 박스가 **둘 다**
           화면 안에 들어가는 가장 큰 값이다. 320 이면 넘쳐서 왼쪽 칸이 스크롤되고,
           스크롤 앵커링이 칸을 굴려 폰 윗 베젤이 잘렸다(실측). 1440×900 이상에서는
           세로 예산이 바닥보다 커서 312~395px 로 커진다.
           16.25rem(260px) = 왼쪽 칸 높이(100dvh-8rem)에서 주소 박스 122 + 사이 간격 8 을 뺀 값.
           예전 9.5rem 은 주소 박스를 안 세어서, 폰이 늘 화면보다 길고 박스는 접힌 선 아래였다(실측). */
        frame === "device"
          ? "w-full shrink-0 max-w-[min(420px,max(280px,calc((100dvh-16.25rem)*0.4618)))]"
          : "w-full max-w-[410px]",
      )}
    >
