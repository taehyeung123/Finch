...
const HOW_SECTIONS: { q: string; body: string[]; points: string[] }[] = [
  {
    ...
    body: [
      ...
      "핀치는 메타가 허용한 정식 방식으로 DM을 보냅니다. 아이디·비밀번호를 넘겨받아 대신 로그인하는 매크로 방식이 아니라서, 계정이 위험해지지 않습니다.",
    ],
...
const IG_ONLY_REASONS = [
  {
    ...
    label: "Instagram",
    ok: true,
    text: "인스타그램은 댓글에 답하는 형태의 자동 DM 발송을 정식으로 허용합니다. 그래서 자동 DM은 편법이 아니라 규칙 안에서 동작합니다.",
  },
  {
    channel: "threads" as const,
    label: "Threads",
    ok: false,
    text: "스레드는 외부 서비스가 DM을 보낼 수 있는 길(API)을 열어 두지 않았습니다. 그래서 자동 DM은 지원되지 않습니다. 스레드는 핀치에서 게시물 발행과 성과 분석으로 쓸 수 있습니다.",
  },
  {
...
const FAQ_ITEMS: FaqItem[] = [
  {
    ...
    a: "핀치는 아이디·비밀번호를 요구하는 매크로가 아니라 메타가 허용한 정식 방식으로 보냅니다. 댓글 1건당 1회 발송 원칙과 하루 발송 상한을 지키고, 키워드에 반응하는 방식이라 무차별 발송이 되지 않습니다. 다만 인스타그램의 스팸 규정은 모든 자동화에 똑같이 적용되므로, 받는 사람이 원하지 않는 대량 발송은 피해야 합니다.",
  },
  {
    q: "스레드나 틱톡에서도 자동 DM이 되나요?",
    a: "자동 DM은 인스타그램 전용 기능입니다. 스레드는 외부 서비스가 DM을 보낼 수 있는 길(API)이 없고, 틱톡은 외부 서비스가 대신 보내는 것을 허용하지 않습니다. 두 채널에서는 자동 DM을 제공하지 않습니다.",
  },
  {
  ...
  },
  {
    ...
    a: "인스타그램이 정한 조건 때문에 비즈니스 또는 크리에이터 계정이 필요합니다. 페이스북 페이지 연결은 필요 없습니다. 개인 계정이라면 전환하는 방법을 시작 과정에서 단계별로 안내해드립니다.",
  },
  {
    q: "무료로 써볼 수 있나요?",
  ...
  "@graph": [
    {
      ...
      description:
        "인스타 자동디엠은 인스타그램 게시물에 특정 키워드 댓글이 달리면 자동으로 DM을 발송하는 기능입니다. 게시물마다 다른 메시지를 설정할 수 있으며 콘텐츠 마케터·광고대행사·인플루언서·쇼핑몰이 사용합니다.",
    },
    {
      "@type": "BreadcrumbList",
      itemListElement: [
    ...
    <>
      ...
      <section className="mx-auto max-w-3xl px-4 pb-10 pt-6 md:px-6">
        ...
        <p className="mt-5 text-[17px] leading-relaxed text-fg-sub">
          인스타 자동디엠(자동 DM)은 게시물에 특정 키워드 댓글이 달리면 자동으로
